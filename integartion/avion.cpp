...
bool avion::ajouter()
{
    ...
    query.prepare("INSERT INTO avions (matricule,compagne,modell) VALUES (:mat,:comp,:model)");
...
{
     QSqlQuery query;
    query.prepare("update AVIONS set matricule=:mat,compagne=:comp,modell=:model where matricule=:mat_modif");
    query.bindValue(":mat",mat);
    query.bindValue(":comp",comp);
    query.bindValue(":model",model);
